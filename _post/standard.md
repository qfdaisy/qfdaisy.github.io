...
この指示は Windows XP 時代のフォルダ構造の問題を避けるために作られたと考えられるが、現代のWindowsにおいてもこれを踏襲する奇妙さについては、別のページで解説したので参照されたい（PRS Pro のプロジェクトフォルダ作成場所[^cdrive]）。この推奨事項をどうしても遂行したいのであれば、Cドライブを避け、Dドライブ直下を利用するべきだろう。
...
詳しい説明は別のページに書いたので参照されたい（音質を考慮したDAISY製作[^quality]）。
...
WCAGのような現代的な国際基準を考慮するなら、各セクションの最初のフレーズとセクションタイトルの記述は一致していることが望ましい[^wcag]。もちろん全国基準に従うだけならWCAGを考慮する必要はないが、参考例と違う文字列を記入しても全国基準に違反しない。
